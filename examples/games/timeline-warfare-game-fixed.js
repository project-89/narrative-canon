...
/**
 * Timeline Warfare - Fixed Version
 * 
...
const {
  ...
  GeminiAdapter,
  MockLLM
} = await import('../../dist/narrative-canon.esm.js');

// Game state
class TimelineWarfareGame {
  constructor(llmAdapter) {
    this.llmAdapter = llmAdapter || new MockLLM();
    this.git = new NarrativeGit(this.llmAdapter);
    ...
    this.isUsingLLM = this.llmAdapter && !(this.llmAdapter instanceof MockLLM);
  }
  ...
  async init() {
    ...
    // Initialize narrative with first commit
  ...
  async displayStatus() {
    ...
    console.log(`🌿 Active Branches: ${this.gameState.activeBranches.length - 1}\n`); // -1 for main
  }
...
Make it cyberpunk themed and relevant to the year ${this.gameState.currentYear}.`;
    ...
    try {
      ...
      // Parse the response into mission format
      // For now, return a structured mission based on the response
      return {
        name: 'Operation ' + this.generateCodename(),
        target: 'Oneirocom Facility',
        description: response.substring(0, 200),
        risk: Math.floor(Math.random() * 4) + 5,
        divergenceGain: Math.floor(Math.random() * 10) + 10,
        successNarrative: 'The mission succeeds. ' + response.substring(200, 400),
        failureNarrative: 'The mission fails. Security was too tight.'
      };
    } catch (error) {
  ...
  generateCodename() {
    const words = ['Shadow', 'Ghost', 'Phoenix', 'Storm', 'Cipher', 'Echo', 'Nexus', 'Prism'];
...
Current year: ${this.gameState.currentYear}`;
    ...
    try {
      const response = await this.llmAdapter.generate(prompt);
      const effects = this.parseCascadeEffects(response);
      
      console.log('\n🌊 CASCADE EFFECTS:\n');
      for (const effect of effects) {
        console.log(`  • ${this.gameState.currentYear + effect.yearsLater}: ${effect.description}`);
        
        // Add to timeline
        await this.git.addAtTime(
          effect.description,
          new Date(`${this.gameState.currentYear + effect.yearsLater}-01-01`),
          'Cascade Effect'
        );
      }
      console.log('');
  ...
  parseCascadeEffects(response) {
    // Simple parsing - in reality would use structured output
    return [
      { yearsLater: 1, description: 'Resistance networks strengthen' },
      { yearsLater: 3, description: 'Oneirocom loses public trust' }
    ];
  }
  ...
  async viewTimeline() {
    ...
    // Group by branch (simplified - would need branch tracking)
    ...
    for (const [branch, commits] of branchMap) {
  ...
  async defendTimeline() {
    if (this.gameState.activeBranches.length <= 1) { // Only main branch
...
Generate their counter-narrative to reconverge the timeline. Should be dystopian and oppressive.`;
  ...
  async attemptWeaving() {
    if (this.gameState.activeBranches.length < 4) { // 3 + main
      console.log('\n❌ Need at least 3 active branches to attempt weaving!\n');
      console.log(`Current branches: ${this.gameState.activeBranches.length - 1}`);
      await this.sleep(2000);
      return;
    }
...
async function startGame() {
  ...
  const game = new TimelineWarfareGame(llmAdapter);
  await game.init();
}
